...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (rol === "administrador") {
    ...
    if ((role === "usuario" || !role) && window.location.href.includes("admin.html")){
    ...
    agregarBtn.addEventListener('click', function() {
        ...
        let nombre = document.getElementById('recipient-name').value.toLowerCase().trim();
        let categoria = document.getElementById('recipient-categoria').value.toLowerCase().trim();
        let descripcion = document.getElementById('message-text').value.toLowerCase().trim();
        let publicado = document.getElementById('recipient-publicado').value.toLowerCase().trim();
        let url = document.getElementById('recipient-url').value.toLowerCase().trim();

        // Crear un objeto Juego con los datos ingresados
        let juego = new Juego(codigo, nombre, categoria, descripcion, publicado, url);
    ...
    function cargarTabla(){
        ...
        juegosGuardados.forEach(element => {
            ...
            <button class="btn btn-outline-warning"><i class="fa-solid fa-star"></i></button>
            </div>
            </td>
            `;
...
function borrar(codigo){
    ...
    let validar = confirm(`Esta seguro/a que quiere eliminar este juego ${juegoAEliminar}?`);
...
function mostrarModalEdit(codigo){
    ...
    let index = juegosGuardados.findIndex((element) => element.codigo == codigoElementUpdate);

    document.getElementById('editar-name').value = juegosGuardados[index].nombre;
    document.getElementById('editar-categoria').value = juegosGuardados[index].categoria;
    document.getElementById('editar-text').value = juegosGuardados[index].descripcion;
    document.getElementById('editar-publicado').value = juegosGuardados[index].publicado;
...
btnEditar.addEventListener('click', function(event){
    ...
    juegosGuardados[index].categoria = document.getElementById('editar-categoria').value;
    juegosGuardados[index].descripcion = document.getElementById('editar-text').value;
    juegosGuardados[index].publicado = document.getElementById('editar-publicado').value;
